...
export function plannerPrompt(options: PlannerPromptOptions): string {
  return [
    ...
    '  Use `"read-write"` if the task modifies the workspace, **runs anything that**',
    '  **writes as a side effect**, or **reaches the network** — a test suite, a build,',
    '  a linter, an install all drop caches and temp files, and a task that cannot act',
    '  cannot verify its own work. `"read-only"` has no network either, so anything',
    '  that fetches, installs, or clones is `"read-write"`. Use `"read-only"` only for',
    '  pure reading: reviewing code, summarizing, answering a question about the repo.',
