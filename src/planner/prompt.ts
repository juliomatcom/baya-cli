...
export function plannerPrompt(options: PlannerPromptOptions): string {
  return [
    ...
    "- `writes` is true if the task modifies the workspace **or runs anything that**",
    "  **writes as a side effect** — a test suite, a build, a linter, an install all",
    "  drop caches and temp files, so they need it. Use false only for pure reading:",
    "  reviewing code, summarizing, answering a question about the repo.",
