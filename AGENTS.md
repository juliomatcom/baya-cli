...
- **Test What Was Decided, Not How It Reads:** The line is not code-vs-UI, it is _signal vs prose_. **Worth asserting:** a run reported success or failure · a warning fired at all, and only in the case that warrants it · a machine-readable contract (`--json`, exit codes) · an invariant that must hold in output (a value that must never leak into a prompt, a field that must always be present). **Not worth asserting:** exact sentences, substring checks that break on a reword, colors and ANSI codes, glyphs, padding, column widths, layout. Before writing one, ask: _does this track a decision the code made, or only the words chosen to express it?_ Track the decision, phrase the assertion as loosely as still catches it, and prefer one assertion on the outcome over five on the wording. Snapshots stay scoped to argv and `--help`.
- **Test in Proportion, and Only Once:** Not all logic earns a test. Weight by what a failure costs: silent wrong answers, data loss, money spent, a security or privacy invariant — worth covering thoroughly. Glue, pass-throughs, and code whose breakage is loud and immediate — usually not. **Never assert the same behaviour at two levels:** a path already exercised end to end by an integration test does not also need a unit test, and duplicating it doubles the edit cost of every future change for no added signal. Reserve unit tests for what integration cannot reach cheaply — error branches, edge cases, pure transformations over many inputs — and let the integration test own the happy path. When both would work, write the one closer to the real invocation, and write it once.
...
4. **Definition of Done:**
   ...
   - [ ] No new test asserts exact wording, color, or layout where the decision behind it would do (see Test What Was Decided).
