...
⚠️ **The prompt is the only lever inside a running group.** One process, one conversation: nothing outside it can stop a member whose dependency has just failed, so a member that does its work anyway is paid for and then discarded by the rule above. The group's prompts are written **before any of it runs**, so an in-group dependency's context entry cannot know how it turned out — it carries `status: "pending"` and says what to do if it did not succeed: report `failed` without attempting it, naming the dependency. It used to claim `status: "ok"` / "Done earlier in this same conversation.", which was a guess printed as a fact. Run `20260903T080018Z`: `scaffold-site` failed on a sandbox with no network, and the two tasks grouped behind it read "(ok)" in their own context, edited the root tooling and ran the root test suite, and had both `ok` results thrown away — with the files left on disk contradicting a state that said `skipped`. `collectUpstreams` in `src/executor/sequential.ts`, rendered by `contextLines` in `src/executor/prompt.ts`.
