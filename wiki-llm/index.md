...
| [execution.md](execution.md)       | How are tasks scheduled, made parallel, locked, isolated, failed, retried, parked, resumed, and interrupted? How are tasks packed into as few provider processes as possible (§Grouping)? How do later tasks reuse what earlier ones learned (§Memory)? Where is the token cost floor, and what has already been measured and rejected (§Cost floor)? |
