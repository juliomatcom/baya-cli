...
| `baya consensus <file\|"prompt">` | Several provider CLIs debate one artifact: moderator writes criteria it keeps to itself, reviewers answer or critique in parallel and blind, moderator reconciles, repeat until nothing blocking is left. Alias `con`. Not a run — no DAG, no lock, invisible to `runs`/`resume`. [consensus.md](consensus.md). | v1     |
...
| `--tools <list>`          | _lean set_    | Capabilities restored on top of each provider's lean tool set: `web`, `agents`, `notebook`, `memories`, `all`. Comma- or space-separated, case-insensitive; unknown name errors. Whole-run override — outranks `providers.<id>.tools` outright. See providers.md §Lean tool sets. |
...
Positional resolves as a path first; a non-existent path is the prompt itself. Empty / C0-binary ⇒ exit `2` before any spawn.

| Flag                    | Default          | Meaning                                                                                                                                                                                                                                                                                         |
| :---------------------- | :--------------- | :---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--providers <models>`  | `defaults.model` | Reviewers, **named by model or alias** — `luna,sonnet`. The CLI follows from the model (`providerForModel`, catalog, `modelAliases`). A bare provider id means that CLI's own default model. Unknown name ⇒ exit `2`. One model per CLI per debate — `reviewers/<provider>/` is the ledger key. |
| `--moderator <model>`   | `planner.model`  | Reconciler, named the same way. May also appear in `--providers`; it then reviews blind before reconciling.                                                                                                                                                                                     |
| `--moderator-model <m>` | —                | Synonym for `--moderator`; both name a model.                                                                                                                                                                                                                                                   |
| `--rounds <n>`          | `5`              | **Ceiling, not a count.** Stops early when nothing `blocker`/`major` is left, or when a round raises no blocking ground the last one did not already fix.                                                                                                                                       |
| `--kind <k>`            | _inferred_       | `question\|plan\|spec\|review\|idea\|prompt`. Skips the classification call **and** fixes both the access posture and whether round 1 answers or critiques (consensus.md §Access posture, §Round 1 answers).                                                                                    |
| `--ledger-budget <n>`   | `8000`           | Chars of a reviewer's own history before compaction.                                                                                                                                                                                                                                            |
| `--output <f>`          | stdout           | Final document. The banner is on stderr, so a piped stdout stays clean.                                                                                                                                                                                                                         |
| `--no-diff`             | off              | Suppress the change summary.                                                                                                                                                                                                                                                                    |

Inherited unchanged: `--json` `--yes` `--verbose` `--quiet` `--log-level` `--no-color` `--no-progress` `--tools`.

**Gate.** Pass 0 runs, then the user confirms before any round. The gate shows the posture, each participant's model, and the remaining call count. ⚠️ Not the criteria: they are the moderator's private yardstick, and a producing run has none (consensus.md §The moderator does not know the answer). `--yes` skips it; a non-TTY without `--yes` exits `2` having spent only pass 0; answering no exits `0`.

⚠️ **Consensus output is quiet by default**, inverting §Run output: provider prose and tool calls are `debug`. `--verbose` restores them. Baya's own lines are unaffected.

⚠️ **Takes no directory lock and isolates nothing.** Reviewers run loose and in parallel with write access to `cwd` in the workspace posture. Warned at the gate; consensus is for reviewing, not developing.
