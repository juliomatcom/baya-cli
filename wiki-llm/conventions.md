...
│  └─ wizard.ts  THE only module that imports @inquirer/prompts
...
11. **`chalk` is imported in exactly one file** — `src/ui/theme.ts`. Everywhere else uses semantic tokens (`theme.ok`, `theme.taskId`). A bare `chalk.green` outside that file is a lint error.
